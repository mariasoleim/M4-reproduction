...
final_dataset <- data.frame(matrix(NA, ncol= 49, nrow = 100000))
...
for (j in 1:length(datasets)){
  for (i in 1:length(datasets[[j]])){
    ...
    final_dataset[n,1] <- datasets[[j]][[i]]$st
    final_dataset[n, 2:(fh+1)]<-ifelse(data.forecast.ses[]<0, 0, data.forecast.ses)
  }
}


write.csv(final_dataset, "final_dataset.csv",sep=";", row.names = FALSE) 
